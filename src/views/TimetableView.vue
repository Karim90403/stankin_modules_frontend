<template>
  <img class="h-7 absolute top-3 left-3 sm:top-20 sm:left-12 sm:h-10" src="../assets/left.png">
  <div class="flex mt-0 bg-transparent overflow-y-scroll ml-10 sm:left-0 w-4/5 sm:w-full sm:ml-0 sm:mt-16 sm:justify-center sm:items-center">
    <div v-for="item in items" :key="item.id" class="flex items-center px-4 py-3 mx-3 rounded-full shadow flex-col sm:h-auto" :class="item.isActive ? 'bg-slate-300' : 'bg-white hover:bg-slate-200'" @click="Activate(item.id)">
      <span class="text-xs sm:text-sm">{{item.dayWeek}}</span>
      <span class="text-xs sm:text-sm">{{item.date}}</span>
    </div>  
  </div>
  <img class="h-7 absolute top-3 right-3 sm:top-20 sm:right-12 sm:h-10"  src="../assets/right.png">


  <div class="flex items-center flex-col mt-12">
    
    <Timetable 
    startTime="12:20" 
    endTime="14:00" 
    subject="Компьютерная графика и геометрия"
    auditory="0408"
    professor="Толок А.В."
    typeSubject="Семинары"
    ></Timetable>
    <Timetable 
    startTime="14:10" 
    endTime="15:50" 
    subject="Архитектура ЭВМ и ВС"
    auditory="0308"
    professor="Саркисова И.О."
    typeSubject="Лабараторные занятия"
    ></Timetable>
    <Timetable 
    startTime="16:00" 
    endTime="17:40" 
    subject="Программирование специализированных вычислительных устройств"
    auditory="210(a)"
    professor="Волкова О.Р."
    typeSubject="Лекции"
    ></Timetable>
  </div>
</template>

<script lang="ts">
import { defineComponent } from 'vue';
import Timetable from '@/components/Timetable.vue'; // @ is an alias to /src
let id = 0
export default defineComponent({
  name: 'HomeView',
  components: {
    Timetable,
  },
  data() {
    return{
      items: [
        {dayWeek: "Пн", date: 12.12, id: id++, isActive: true},
        {dayWeek: "Вт", date: 13.12, id: id++, isActive: false},
        {dayWeek: "Ср", date: 14.12, id: id++, isActive: false},
        {dayWeek: "Чт", date: 15.12, id: id++, isActive: false},
        {dayWeek: "Пт", date: 16.12, id: id++, isActive: false},
        {dayWeek: "Сб", date: 17.12, id: id++, isActive: false},
        {dayWeek: "Вс", date: 18.12, id: id++, isActive: false},
      ]
    }
  },
  methods:{
    Activate( id: number ):void {
      this.items.forEach(item => {
        if (item.id == id) {
          item.isActive = true;
        } else if (item.id != id && item.isActive == true){
          item.isActive = false;
        }
      });
    }
  }
});
</script>